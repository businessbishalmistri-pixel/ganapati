...
import { X, Check, ShoppingBag, Plus, Minus, Package, AlertCircle } from 'lucide-react';
...
export function VariantSelectorSheet({ isOpen, onClose, product }) {
  ...
  // Sync when product or sheet opens
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="fixed inset-x-0 bottom-0 sm:inset-0 sm:flex sm:items-center sm:justify-center p-0 sm:p-4 pointer-events-none z-10">
        <div 
          className="w-full sm:max-w-md bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl border border-slate-100 flex flex-col max-h-[75vh] sm:max-h-[85vh] overflow-hidden pointer-events-auto animate-slide-up sm:animate-scale-in"
          onClick={(e) => e.stopPropagation()}
        >
          
          {/* Mobile Top Drag Handle */}
          <div className="w-10 h-1 bg-slate-300 rounded-full mx-auto mt-2.5 mb-1 sm:hidden flex-shrink-0" />
          ...
          <div className="p-3.5 sm:p-4 border-b border-slate-100 flex items-center justify-between gap-3 bg-slate-50/70">
            <div className="flex items-center gap-3 min-w-0">
              {/* Product Thumbnail */}
              <div className="w-12 h-12 rounded-lg bg-white border border-slate-200/80 overflow-hidden flex-shrink-0 flex items-center justify-center">
          ...
          <div className="p-3.5 sm:p-4 overflow-y-auto space-y-2 flex-1 no-scrollbar">
          ...
          {/* Sticky Bottom Footer: Quantity Stepper + Add to Cart Button */}
          <div className="p-3.5 sm:p-4 border-t border-slate-100 bg-white space-y-2.5 flex-shrink-0">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center border border-slate-200 rounded-lg p-0.5 bg-slate-50">
                <button
                  ...
                  className="w-7 h-7 rounded flex items-center justify-center text-slate-700 hover:bg-white disabled:opacity-30 transition-colors cursor-pointer"
                  title="Decrease"
                >
            ...
            <button
              ...
              className={`w-full py-2.5 px-4 rounded-xl text-xs sm:text-sm font-bold flex items-center justify-center gap-2 transition-all shadow-md active:scale-98 cursor-pointer ${
                isCurrentOutOfStock
